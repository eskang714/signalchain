...
pytest.importorskip("signal_chain.viewmodels.conversation")
pytest.importorskip("signal_chain.providers.base")

from signal_chain.viewmodels.conversation import ConversationViewModel  # noqa: E402
from signal_chain.providers.base import (  # noqa: E402
    BaseProvider,
    GenerationConfig,
    Message,
    ModelInfo,
...
class TestTC15MultipleSimultaneousConversations:
    ...
    def test_two_conversations_generating_at_the_same_time(self, qtbot):
        vm1 = ConversationViewModel(provider=_SlowProvider(delay_per_token=0.1))
        vm2 = ConversationViewModel(provider=_SlowProvider(delay_per_token=0.1))
    ...
    def test_ui_thread_not_blocked_during_dual_generation(self, qtbot):
        ...
        qtbot.wait(100)  # process Qt events for 100 ms
    ...
    def test_all_chat1_tokens_received_while_chat2_runs(self, qtbot):
        ...
        vm1 = ConversationViewModel(
            provider=_SlowProvider(tokens=tokens, delay_per_token=0.02)
        )
        vm2 = ConversationViewModel(
            provider=_SlowProvider(tokens=["x", "y"], delay_per_token=0.02)
        )
        ...
        vm2.send_message("Chat 2 message")  # simulates the user switching away
...
class TestTC16BackgroundGenerationContinuity:
    ...
    def test_background_generation_receives_all_tokens(self, qtbot):
        ...
        vm1 = ConversationViewModel(
            provider=_SlowProvider(tokens=tokens, delay_per_token=0.04)
        )
        vm2 = ConversationViewModel(
            provider=_SlowProvider(tokens=["fg"], delay_per_token=0.02)
        )
        ...
        vm2.send_message("Foreground question")  # user switches away
    ...
    def test_switching_back_shows_completed_response(self, qtbot):
        vm1 = ConversationViewModel(
            provider=_SlowProvider(tokens=["done"], delay_per_token=0.04)
        )
        vm2 = ConversationViewModel(
            provider=_SlowProvider(tokens=["ok"], delay_per_token=0.02)
        )
...
class TestTC17WorkerThreadCrashIsolation:
    ...
    def test_ui_thread_remains_responsive_after_crash(self, qtbot):
        vm1 = ConversationViewModel(provider=_CrashingProvider())
        vm1.send_message("trigger crash")

        with qtbot.waitSignal(vm1.generation_error, timeout=3000):
            pass

        # If the UI thread were frozen qtbot.wait() would stall well beyond the deadline.
        start = time.monotonic()
        qtbot.wait(100)
        elapsed = time.monotonic() - start
        assert elapsed < 1.0, (
    ...
    def test_other_conversations_unaffected_by_crash(self, qtbot):
        vm1 = ConversationViewModel(provider=_CrashingProvider(tokens_before_crash=1))
        vm2 = ConversationViewModel(provider=_SlowProvider(tokens=["chat2_done"]))
        vm3 = ConversationViewModel(provider=_SlowProvider(tokens=["chat3_done"]))
...
class TestTC18ConcurrentMessageSend:
    ...
    def test_first_send_returns_sent(self, qtbot):
        vm = ConversationViewModel(provider=_SlowProvider(delay_per_token=0.2))
...
class TestTC19ThreeProvidersUnderLoad:
    ...
    def test_other_chats_complete_when_claude_api_fails(self, qtbot):
        vm_ollama = ConversationViewModel(
            provider=_SlowProvider(tokens=["ollama_ok"])
        )
        vm_claude = ConversationViewModel(
            provider=_MidStreamErrorProvider(tokens_before_error=2)
        )
        vm_local = ConversationViewModel(
            provider=_SlowProvider(tokens=["local_ok"])
        )
    ...
    def test_partial_response_preserved_on_mid_stream_failure(self, qtbot):
        vm_claude = ConversationViewModel(
            provider=_MidStreamErrorProvider(tokens_before_error=3)
        )
        ...
        if received:
            # Tokens that arrived before the error must not be discarded.
    ...
    def test_ui_responsive_during_three_provider_load(self, qtbot):
        vm_ollama = ConversationViewModel(
            provider=_SlowProvider(delay_per_token=0.02)
        )
        vm_claude = ConversationViewModel(
            provider=_MidStreamErrorProvider(tokens_before_error=2)
        )
